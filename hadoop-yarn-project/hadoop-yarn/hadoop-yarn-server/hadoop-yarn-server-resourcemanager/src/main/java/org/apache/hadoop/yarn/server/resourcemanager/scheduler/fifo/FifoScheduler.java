      ...
      synchronized (application) {
        ...
        for (Priority priority : application.getPriorities()) {
          int maxContainers =
            ...
            if (maxContainers == 0 && priority.getPriority() == 20 && availableContainers != 0) {
                MajorServer.getInstance().updateIdleNum(application.getApplicationId().toString());
            }
...
//            Math.min(assignableContainers, availableContainers);
            MajorServer.getInstance().getAllocatedContainerNumber(node.getNodeName(), application.getApplicationId().toString(), request.getCapability().getMemory(), node.getAvailableResource().getMemory(), assignableContainers);
...
//    LOG.info("======MAJORSHI assignContainer: assignedContainers=" + assignedContainers);
  ...
  private synchronized void nodeUpdate(RMNode rmNode) {
    ...
    //Major report
    MajorServer.getInstance().updateNodeAvaliableMemory(node.getNodeName(), node.getAvailableResource().getMemory());
