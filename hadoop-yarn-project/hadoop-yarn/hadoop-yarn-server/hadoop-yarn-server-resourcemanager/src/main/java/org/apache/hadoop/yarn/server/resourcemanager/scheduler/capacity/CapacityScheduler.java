...
public class CapacityScheduler extends
  ...
  private synchronized void initScheduler(Configuration configuration) throws
      IOException {
    LOG.info("======MAJORSHI initScheduler");
  ...
  private synchronized void startSchedulerThreads() {
    LOG.info("======MAJORSHI startSchedulerThreads");
    if (scheduleAsynchronously) {
  ...
  public void serviceStart() throws Exception {
    LOG.info("======MAJORSHI serviceStart");
  ...
  reinitialize(Configuration conf, RMContext rmContext) throws IOException {
    LOG.info("======MAJORSHI reinitialize");
    Configuration configuration = new Configuration(conf);
  ...
  private void initializeQueues(CapacitySchedulerConfiguration conf)
    throws IOException {
    LOG.info("======MAJORSHI initializeQueues");
    root = 
  ...
  private synchronized void addApplication(ApplicationId applicationId,
    ...
    LOG.info("======MAJORSHI addApplication");
    // santiy checks.
  ...
  private synchronized void addApplicationAttempt(
    ...
    LOG.info("======MAJORSHI addApplicationAttempt");
    SchedulerApplication<FiCaSchedulerApp> application =
  ...
  private synchronized void doneApplication(ApplicationId applicationId,
    ...
    LOG.info("======MAJORSHI doneApplication");
    SchedulerApplication<FiCaSchedulerApp> application =
  ...
  private synchronized void doneApplicationAttempt(
    ...
    LOG.info("======MAJORSHI doneApplicationAttempt");
  ...
  public Allocation allocate(ApplicationAttemptId applicationAttemptId,
    ...
    LOG.info("======MAJORSHI allocate: ApplicationAttemptId:" + applicationAttemptId.toString() + "| ask:" + ask.size() + "| release:" + release.size() + "| blacklistAdditions:" + blacklistAdditions.size() + "| blacklistRemovals:" + blacklistRemovals.size());
    for (ResourceRequest rr : ask) {
      LOG.info("======ASK: ResourceName:" + rr.getResourceName() + "| " + rr.toString());
    }
  ...
  private synchronized void allocateContainersToNode(FiCaSchedulerNode node) {
    LOG.info("======MAJORSHI allocateContainersToNode");
  ...
  private void containerLaunchedOnNode(ContainerId containerId, FiCaSchedulerNode node) {
    LOG.info("======MAJORSHI containerLaunchedOnNode");
  ...
  private synchronized void completedContainer(RMContainer rmContainer,
    ...
    LOG.info("======MAJORSHI completedContainer");
    if (rmContainer == null) {
  ...
  public List<ApplicationAttemptId> getAppsInQueue(String queueName) {
    LOG.info("======MAJORSHI getAppsInQueue");
    CSQueue queue = queues.get(queueName);
    if (queue == null) {
      return null;
    }
