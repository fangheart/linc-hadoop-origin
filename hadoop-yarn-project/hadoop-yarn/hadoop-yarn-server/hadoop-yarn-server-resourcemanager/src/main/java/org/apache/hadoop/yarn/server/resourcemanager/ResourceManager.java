...
import org.apache.hadoop.yarn.server.major.MajorServer;
...
public class ResourceManager extends CompositeService implements Recoverable {
  ...
  public class RMActiveServices extends CompositeService {
    ...
    protected void serviceStart() throws Exception {
      ...
      rmStore.start();
      LOG.info("======MAJORSERVER ready to run()");
      if (!MajorServer.getInstance().getisOpen()) {
        MajorServer.getInstance().run();
        LOG.info("======MAJORSERVER run()");
      }
      if(recoveryEnabled) {
        try {
