...
import org.apache.hadoop.mapreduce.v2.app.Major.MajorClient;
...
public class RMContainerAllocator extends RMContainerRequestor
  ...
  private static final HashSet<TaskAttemptId> hasReportTask = new HashSet<TaskAttemptId>();
  ...
  */
  ...
  protected synchronized void heartbeat() throws Exception {
    ...
    //Major start
    Map<String,List<String>> taskinfo = new HashMap<String,List<String>>();
    int c = 0, m = 0;
    for (TaskAttemptId taskid:  scheduledRequests.maps.keySet()) {
      if (hasReportTask.contains(taskid)) continue;
      ArrayList<String> ar = new ArrayList<String>();
      ContainerRequest cr = scheduledRequests.maps.get(taskid);
      if (c == 0 || m == 0) {
        c = cr.capability.getVirtualCores();
        m = cr.capability.getMemory();
      }
      for (String hosts : scheduledRequests.maps.get(taskid).hosts) {
        ar.add(hosts);
      }
      taskinfo.put(taskid.toString(), ar);
      hasReportTask.add(taskid);
    }
    if (taskinfo.size() != 0)
    MajorClient.updateTaskInfo(applicationId.toString(), taskinfo, taskinfo.size(), c, m);
    //Major end
  ...
  protected synchronized void handleEvent(ContainerAllocatorEvent event) {
    ...
    if (event.getType() == ContainerAllocator.EventType.CONTAINER_REQ) {
      ...
      LOG.info("===== MAJORSHI AM handleEvent:" + reqEvent.toString());
      StringBuilder sb = new StringBuilder();
      sb.append("===== MAJORSHI AM handleEvent hosts:");
      for (String s : reqEvent.getHosts()) {
        sb.append(s);
        sb.append("|");
      }
  ...
  class ScheduledRequests {
    ...
    private ContainerRequest getContainerReqToReplace(Container allocated) {
      ...
      if (PRIORITY_FAST_FAIL_MAP.equals(priority)) {
        ...
        //Major start
        StringBuilder sb = new StringBuilder();
        sb.append("===== MAJORSHI mapsHostMapping:" + host);
        for (TaskAttemptId tid : list) {
    ...
    private void assignMapsWithLocality(List<Container> allocatedContainers) {
      // Major start
//      HashMap<String, TaskAttemptId> taskidtotask = new HashMap<String, TaskAttemptId>();
//      for (TaskAttemptId tid : maps.keySet()) {
//        taskidtotask.put(tid.toString(), tid);
//      }
//
...
//      }
//      Map<String, String> taskmap = MajorClient.getTaskContainerInfo(applicationId.toString(), cids);
//      for (String cid : taskmap.keySet()) {
//        LOG.info("!!! container: " + cid + "| task:" + taskmap.get(cid));
//      }
...
//        String host = allocated.getNodeId().getHost();
//        if (taskmap.containsKey(allocated.getId().toString())) {
//          String taskid = taskmap.get(allocated.getId().toString());
//          if (taskidtotask.containsKey(taskid)) {
//            TaskAttemptId t = taskidtotask.get(taskid);
//            if (maps.containsKey(t)) {
//              ContainerRequest assigned = maps.remove(t);
//              containerAssigned(allocated, assigned);
//              it.remove();
//              JobCounterUpdateEvent jce =
//                      new JobCounterUpdateEvent(assigned.attemptID.getTaskId().getJobId());
...
//                hostLocalAssigned++;
//                LOG.info("!!!! hostlocal:" + host + "|" + allocated.getId().toString());
//              } else {
//                jce.addCounterUpdate(JobCounter.RACK_LOCAL_MAPS, 1);
//                rackLocalAssigned++;
//                LOG.info("!!!! racklocal:" + RackResolver.resolve(host).getNetworkLocation() + "|" + allocated.getId().toString());
//              }
//              eventHandler.handle(jce);
//            }
//          }
