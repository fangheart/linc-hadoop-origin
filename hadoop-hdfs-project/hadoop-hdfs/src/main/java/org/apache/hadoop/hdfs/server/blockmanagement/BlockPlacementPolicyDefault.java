...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
    ...
    private HashSet<String> lastBlockReplicationLocations;  //Majoradd
    private HashSet<String> nextBlockFirstReplicationLocations;
    private HashSet<String> nextBlockSecondReplicationLocations;
    private HashSet<String> nextBlockThirdReplicationLocations;
    private HashSet<String> majorClusters;
    ...
    public void initialize(Configuration conf,  FSClusterStats stats,
        ...
        this.lastBlockReplicationLocations = new HashSet<String>();
        this.nextBlockFirstReplicationLocations = new HashSet<String>();
        this.nextBlockSecondReplicationLocations = new HashSet<String>();
        this.nextBlockThirdReplicationLocations = new HashSet<String>();
        this.majorClusters = new HashSet<String>();
        for (int i = 1; i < 6; i++) {
            this.majorClusters.add("linc-" + i);
        }
    }
    ...
    private DatanodeStorageInfo[] chooseTarget(int numOfReplicas,
                                               ...
                                               long blocksize,
                                               StorageType storageType) {
        LOG.info("=====MAJORSHI: TAG-3-in: numOfReplicas:" + numOfReplicas + "| chosenStorage:" + chosenStorage.size());
        ...
        LOG.info("=====MAJORSHI: TAG-3-out");
        if (writer != null) {
            LOG.info("!!! writer:" + writer.getName());
        }
        if (localNode != null) {
            LOG.info("!!! localNode:" + localNode.getName());
        }
        for (DatanodeStorageInfo ds : results) {
            Iterator<BlockInfo> bi = ds.getBlockIterator();
            while (bi.hasNext()) {
                BlockInfo binfo = bi.next();
                LOG.info("!!!! " + binfo.getBlockName());
            }
        }
    ...
    private Node chooseTarget(int numOfReplicas,
        ...
        LOG.info("=====MAJORSHI TAG-2-in: numOfReplicas:" + numOfReplicas + "| blocksize:" + blocksize + "| maxNodesPerRack:" + maxNodesPerRack + "| results:" + results.size());
        if (numOfReplicas == 0 || clusterMap.getNumOfLeaves()==0) {
            return writer;
        }
        //Major add
        boolean isMajorClusters = false;
        HashSet<String> cmpset = new HashSet<String>();
        HashMap<String, DatanodeStorageInfo[]> nodeDSMap = new HashMap<String, DatanodeStorageInfo[]>();
        for (Node n : clusterMap.getDatanodesInRack("/default-rack")) {
            DatanodeDescriptor dd = (DatanodeDescriptor)n;
            cmpset.add(dd.getHostName());
            nodeDSMap.put(dd.getHostName(), dd.getStorageInfos());
        }
        cmpset.retainAll(majorClusters);
        if (cmpset.size() == majorClusters.size()) {
            isMajorClusters = true;
        }
        HashSet<String> nowSet = new HashSet<String>();
        //major end
        int totalReplicasExpected = numOfReplicas + results.size();
        for (DatanodeStorageInfo ds : results) {
            LOG.info("=========MAJORSHI ADD:|StorageID: " + ds.getStorageID() + "|StorageType: " + ds.getStorageType() + "|Capacity: " + ds.getCapacity() + "|State: " + ds.getState() + "|DS: " + ds.getDatanodeDescriptor() + "|BlockPoolUsed: " + ds.getBlockPoolUsed());
        }
        ...
        //majorshi add
        int nor = numOfResults;
        try {
            if (numOfResults == 0) {
                boolean ok = false;
                if (isMajorClusters) {
                    ArrayList<String> firstRndList = new ArrayList<String>();
                    if (nextBlockFirstReplicationLocations.size() == 0) {
                        firstRndList.addAll(nodeDSMap.keySet());
                        Collections.shuffle(firstRndList);
                    } else {
                        firstRndList.addAll(nextBlockFirstReplicationLocations);
                        Collections.shuffle(firstRndList);
                    }
                    for (String first : firstRndList) {
                        DatanodeStorageInfo[] dss = nodeDSMap.get(first);
                        for(int i = 0; i < dss.length; i++) {
                            final int newExcludedNodes = addIfIsGoodTarget(dss[i],
                                    excludedNodes, blocksize, maxNodesPerRack, considerLoad, results,
                                    avoidStaleNodes, storageType);
                            if (newExcludedNodes >= 0) {
                                numOfReplicas--;
                                writer = clusterMap.getNode(dss[i].getDatanodeDescriptor().getNetworkLocation());
                                nowSet.add(first);
                                ok = true;
                    ...
                    if (numOfReplicas == 0) {
                        lastBlockReplicationLocations = nowSet;
                        return writer;
                    }
                }
                if (!ok) {
                    ...
                    if (numOfResults != results.size()) {
                        LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 1:chooseLocalStorage");
                        nor = results.size();
                    }
                    if (--numOfReplicas == 0) {
                        return writer;
            ...
            if (numOfResults <= 1) {
                ...
                if (isMajorClusters) {
                    ...
                    ArrayList<String> secRndList = new ArrayList<String>();
                    secSet.addAll(nextBlockSecondReplicationLocations);
                    if (secSet.size() == 0) {
                        secSet.addAll(nodeDSMap.keySet());
                        secSet.removeAll(nowSet);
                        secRndList.addAll(secSet);
                        Collections.shuffle(secRndList);
                    } else {
                        secRndList.addAll(secSet);
                        Collections.shuffle(secRndList);
                    }
                    for (String second : secRndList){
                        DatanodeStorageInfo[] dss = nodeDSMap.get(second);
                        for(int i = 0; i < dss.length; i++) {
                            final int newExcludedNodes = addIfIsGoodTarget(dss[i],
                                    excludedNodes, blocksize, maxNodesPerRack, considerLoad, results,
                                    avoidStaleNodes, storageType);
                    ...
                    if (numOfReplicas == 0) {
                        HashSet<String> nextfirst = new HashSet<String>();
                        nextfirst.addAll(nowSet);
                        HashSet<String> rm = new HashSet<String>();
                        rm.addAll(nowSet);
                        rm.retainAll(lastBlockReplicationLocations);
                        nextfirst.removeAll(rm);
                        nextBlockFirstReplicationLocations = nextfirst;
                        HashSet<String> nextsecond = new HashSet<String>();
                        nextsecond.addAll(nodeDSMap.keySet());
                        nextsecond.removeAll(nextfirst);
                        nextsecond.removeAll(rm);
                        nextBlockSecondReplicationLocations = nextsecond;
                        lastBlockReplicationLocations = nowSet;
                        return writer;
                    }
                }
                if (!ok) {
                    ...
                    //majorshi add
                    if (numOfResults != results.size()) {
//              LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 2:chooseRemoteRack");
                        nor = results.size();
                    }
                    if (--numOfReplicas == 0) {
                        return writer;
            ...
            if (numOfResults <= 2) {
                ...
                if (isMajorClusters) {
                    ...
                    ArrayList<String> thirdRndList = new ArrayList<String>();
                    thirdSet.addAll(nextBlockThirdReplicationLocations);
                    if (thirdSet.size() == 0) {
                        thirdSet.addAll(nodeDSMap.keySet());
                        thirdSet.removeAll(nowSet);
                        thirdRndList.addAll(thirdSet);
                        Collections.shuffle(thirdRndList);
                    } else {
                        thirdSet.removeAll(nowSet);
                        thirdRndList.addAll(thirdSet);
                        Collections.shuffle(thirdRndList);
                    }
                    for (String third : thirdRndList){
                        DatanodeStorageInfo[] dss = nodeDSMap.get(third);
                        for(int i = 0; i < dss.length; i++) {
                            final int newExcludedNodes = addIfIsGoodTarget(dss[i],
                                    excludedNodes, blocksize, maxNodesPerRack, considerLoad, results,
                                    avoidStaleNodes, storageType);
                    ...
                    if (numOfReplicas == 0) {
                        ...
                        nextsecond.removeAll(rm);
                        nextBlockSecondReplicationLocations = nextsecond;
                        nextBlockThirdReplicationLocations = nextsecond;
                        lastBlockReplicationLocations = nowSet;
                        return writer;
                    }
                }
                if (!ok) {
                    ...
                    if (clusterMap.isOnSameRack(dn0, dn1)) {
                        ...
                        //majorshi add
                        if (numOfResults != results.size()) {
                            LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 3:chooseRemoteRack");
                            nor = results.size();
                        }
                    } else if (newBlock){
                        ...
                        //majorshi add
                        if (numOfResults != results.size()) {
                            LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 4:chooseLocalRack");
                            nor = results.size();
                        }
                    } else {
                        ...
                        //majorshi add
                        if (numOfResults != results.size()) {
                            LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 5:chooseLocalRack");
                            nor = results.size();
                        }
                    }
                    if (--numOfReplicas == 0) {
            ...
            //majorshi add
            if (numOfResults != results.size()) {
                LOG.info("=====MAJORSHI results change: " + nor + " - " + results.size() + " after 6:chooseRandom");
                nor = results.size();
            }
